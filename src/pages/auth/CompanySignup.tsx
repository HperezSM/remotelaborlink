...
const CompanySignup = () => {
  const [form, setForm] = useState({
    companyName: "", email: "", password: "", website: "", size: "", industry: "", howHeard: ""
  });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === "email") {
      setEmailError(value && isFreeEmailDomain(value) ? "Please use your company email address" : "");
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.companyName || !form.email || !form.password) return;
    if (form.password.length < 6) {
      toast({ title: "Password must be at least 6 characters", variant: "destructive" });
      return;
    }
    setLoading(true);
    const { error } = await signUpCompany(form.email, form.password, form.companyName, form.website, form.size, form.industry, form.howHeard);
    setLoading(false);
    if (error) {
      toast({ title: "Signup failed", description: (error as any).message, variant: "destructive" });
    } else {
      toast({ title: "Account created!", description: "Your account is under review. We'll notify you once approved." });
          ...
          <div className="card-surface p-8">
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div>
                ...
                <input name="password" type="password" value={form.password} onChange={handleChange} placeholder="••••••••" required minLength={6} className={inputClass} />
              </div>
              <div>
              ...
              <Button type="submit" disabled={loading || !!emailError} className="w-full bg-primary text-primary-foreground font-bold py-3.5 hover:opacity-90">
